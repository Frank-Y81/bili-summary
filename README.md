...
1. 侧边栏右上角 `⚙` → 「厂商管理」→ 「+ 新增厂商」
2. **接口地址填到 `/v1` 那一层**，编辑器里有「实际请求：…」实时预览，照着看就不会填错。常用值：
...
- 逆向网页版 AI（走网页协议 + 登录态）这条线**没做**
- 逆向接口随时可能失效；本项目不做自动重试

## 开发提示

- `src/providers.ts` 顶部 `DRY_RUN = true` 时走假流向（不花钱，能验流式渲染与整条链路）
- 类型检查单独跑：`npx vue-tsc -b --force`